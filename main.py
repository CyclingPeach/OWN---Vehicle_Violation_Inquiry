...
class Home(object):
    ...
    '''
        读取用户信息csv文件
        编号 姓名 性别 年龄 车型 省份 号牌 驾驶证号 驾驶证档案编号 发动机号后六位
    '''
    def get_user_csv():
    ...
    def __init__(self):
        ...
        ## 四个功能按钮分别关联 四个 函数(函数?/类?迷？😅)，并跳转到指定功能界面
...
class First(object):
    ...
    def __init__(self):
        ...
        s_1 = StringVar()
        s_2 = StringVar()
        entry_1 = Entry(self.root, textvariable=s_1, width=15);  entry_1.place(x=225,y=70);    s_1.set('请输入号牌号码')
        entry_2 = Entry(self.root, textvariable=s_2, width=23);  entry_2.place(x=170,y=105);    s_2.set('请输入发动机号后六位')
